...
# ====== CONFIG ======
# run this script from: src/scripts
# raw datasets:
#   ../../dataset/pyro_sdis_yolo
#   ../../dataset/dfire
...
OUT_ROOT = Path("../../dataset/fire_smoke_combined_new")
...
def copy_pair(src_img, src_lbl, dst_img_dir, dst_lbl_dir, class_remap):
    ...
    for ext in ("*.jpg", "*.jpeg", "*.png"):
        for img_path in img_dir.glob(ext):
            ...
            if not lbl_path.exists():
                continue
            copy_pair(img_path, lbl_path, out_img_dir, out_lbl_dir, class_remap)


def main():
    ensure_dirs()

    # RAW: 0=smoke, 1=fire  →  GLOBAL: 0=fire, 1=smoke
    dfire_remap = {0: 1, 1: 0}
    pyro_remap  = {0: 1, 1: 0}
    ...
    print("   GLOBAL CLASSES: 0=fire, 1=smoke")
